...
def get_cutadapt_args(region, length, primer_status):
    ...
    if region == "V3-V4":
        ...
        # 300 bp — R1 only reaches ~126bp into V4 after 515F trim,
        # never reaches 806R_RC. Only R2 can read through into 515F_RC.
        if included:
            return ["-g", PRIMERS["515F"], "--overlap", "16",
    ...
    if region == "V4-V5":
        ...
        # 300 bp — R2 after 806R trim has only ~58bp of V4
        if included:
            return [
                "-g", PRIMERS["515F"],
                "-G", PRIMERS["806R"],
...
def format_case_label(case_num, args, region, length, primer_status):
    """Build a human-readable label for the detected case."""
    flag_str = _format_args(args)
    return (
        f"Case {case_num}: {region} / {length} bp / {primer_status} "
        f"→ `{flag_str}`"
    )
...
def _cli_process_sample(s_name, r1, r2, n_bases, limit, threads, outdir, dry_run):
    """QC + trim one sample for CLI. Returns (output_text, failed_bool)."""
    out = [f"--- {s_name} ---"]
    existing = glob.glob(os.path.join(outdir, f"{s_name}_*_trimmed_R1.fastq.gz"))
    if existing:
        out.append(f"SKIP: output already exists ({os.path.basename(existing[0])})")
        return "\n".join(out), False
    r1_stats = get_comprehensive_stats(r1, n_bases, limit)
    r2_stats = get_comprehensive_stats(r2, n_bases, limit)
    r1_prefix, r1_avg, _, r1_q30, r1_q25, r1_q20, r1_total = r1_stats
    r2_prefix, r2_avg, _, r2_q30, r2_q25, r2_q20, r2_total = r2_stats
    out.append(f"R1: {r1_total:,} reads, avg {r1_avg:.1f} bp, Q30@{r1_q30} Q25@{r1_q25} Q20@{r1_q20}")
    out.append(f"    prefix: {r1_prefix}")
    out.append(f"R2: {r2_total:,} reads, avg {r2_avg:.1f} bp, Q30@{r2_q30} Q25@{r2_q25} Q20@{r2_q20}")
    out.append(f"    prefix: {r2_prefix}")
    vregion = fast_identify_vregion(r1_prefix, r2_prefix)
    if vregion is not None:
        out.append(f"V-region: {vregion} (from primer prefix)")
    else:
        vregion = identify_vregion(r1, r2, ECOLI_REF, threads=threads)
        out.append(f"V-region: {vregion}")
    if vregion not in VALID_V4_REGIONS:
        out.append(f"ERROR: unsupported V-region '{vregion}' — skipping")
        return "\n".join(out), True
    det_length = detect_read_length(r1_avg)
    det_primers = detect_primer_status(r1_prefix, vregion, r2_prefix)
    out.append(f"Read length: {det_length} bp, Primers: {det_primers}")
    ca_args, case_num, error = get_cutadapt_args(vregion, det_length, det_primers)
    if error:
        out.append(f"Case {case_num}: {error}")
        out.append(f"ERROR: {error}")
        return "\n".join(out), True
    r2_pad = detect_r2_pad(r2)
    if r2_pad > 0 and "-G" not in (ca_args or []):
        out.append(f"R2 5' pad detected: {r2_pad} bp (dark cycle)")
        if ca_args is None:
            ca_args = []
        ca_args = ["-U", str(r2_pad)] + ca_args
    out.append(f"Case {case_num}: {_format_args(ca_args) if ca_args else '(no action)'}")
    tag = VREGION_TAG[vregion]
    r1_out = os.path.join(outdir, f"{s_name}_{tag}_{det_length}bp_trimmed_R1.fastq.gz")
    r2_out = os.path.join(outdir, f"{s_name}_{tag}_{det_length}bp_trimmed_R2.fastq.gz")
    if not ca_args:
        out.append("No trimming needed — copying input files as-is.")
        os.makedirs(outdir, exist_ok=True)
        shutil.copy2(r1, r1_out)
        shutil.copy2(r2, r2_out)
        out.append(f"Output: {r1_out}, {r2_out}")
        return "\n".join(out), False
    cmd = ["cutadapt"] + ca_args + ["-o", r1_out, "-p", r2_out, r1, r2]
    out.append(f"Command: {' '.join(cmd)}")
    if dry_run:
        out.append("(dry run)")
        return "\n".join(out), False
    os.makedirs(outdir, exist_ok=True)
    ok, stderr = run_cutadapt(ca_args, r1, r2, r1_out, r2_out, threads=threads)
    if not ok:
        out.append(f"ERROR: cutadapt failed:\n{stderr}")
        return "\n".join(out), True
    r1_after = fastq_avg_len(r1_out)
    r2_after = fastq_avg_len(r2_out)
    out.append(f"R1 avg len: {r1_avg:.1f} -> {r1_after:.1f}")
    out.append(f"R2 avg len: {r2_avg:.1f} -> {r2_after:.1f}")
    out.append(f"Output: {r1_out}, {r2_out}")
    return "\n".join(out), False
...
def cli_main():
    """Command-line interface for testing QC + cutadapt extraction."""
    import argparse

    parser = argparse.ArgumentParser(
        description="Paired-end FASTQ QC & V4 extraction (CLI mode)")
    parser.add_argument("files", nargs="+", help="FASTQ files (*_R1.fastq.gz *_R2.fastq.gz)")
    parser.add_argument("-o", "--outdir", default=".", help="Output directory (default: .)")
    parser.add_argument("-n", "--n-bases", type=int, default=20, help="Prefix length (default: 20)")
    parser.add_argument("--limit", type=int, default=50000, help="Read limit for QC (default: 50000)")
    parser.add_argument("--dry-run", action="store_true", help="Show cutadapt command without running")
    _auto_jobs, _auto_threads = _autodetect_resources()
    parser.add_argument("-t", "--threads", type=int, default=_auto_threads,
                        help=f"Threads per tool: cutadapt -j, bbmap threads= (default: auto={_auto_threads})")
    parser.add_argument("-j", "--jobs", type=int, default=_auto_jobs,
                        help=f"Parallel samples to process simultaneously (default: auto={_auto_jobs})")
    args = parser.parse_args()
    print(f"Resources: {os.cpu_count()} CPUs — using -j {args.jobs} -t {args.threads}")
    ...
    for fpath in args.files:
        ...
        match = re.search(r"(.+)_(R?[12])(?:[_.]|\.fastq).*\.gz$", basename)
        if match:
            s_name = match.group(1)
            r_type = match.group(2) if match.group(2).startswith("R") else "R" + match.group(2)
            if s_name not in samples:
                samples[s_name] = {"R1": None, "R2": None}
            samples[s_name][r_type] = fpath
        else:
            unmatched.append(basename)

    if unmatched:
        print(f"WARNING: Could not parse sample name from: {', '.join(unmatched)}")
    if not samples:
        print("ERROR: No valid paired-end files found.")
        return 1

    # Validate all samples have both R1 and R2
    for s_name, paths in sorted(samples.items()):
        if not paths["R1"] or not paths["R2"]:
            missing = "R1" if not paths["R1"] else "R2"
            print(f"ERROR: {s_name} is missing {missing}")
            return 1

    print(f"=== {len(samples)} sample(s) detected ===\n")

    failed = 0
    sorted_names = sorted(samples)
    results_map = {}
    with ProcessPoolExecutor(max_workers=args.jobs) as executor:
        future_to_name = {
            executor.submit(
                _cli_process_sample,
                s_name, samples[s_name]["R1"], samples[s_name]["R2"],
                args.n_bases, args.limit, args.threads, args.outdir, args.dry_run,
            ): s_name
            for s_name in sorted_names
        }
        for future in as_completed(future_to_name):
            s_name = future_to_name[future]
            results_map[s_name] = future.result()

    for s_name in sorted_names:
        output, did_fail = results_map[s_name]
        print(output)
        print()
        if did_fail:
            failed += 1
    ...
    if failed:
        print(f"{failed} sample(s) failed.")
        return 1
